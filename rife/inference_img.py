...
def parse_args():
    ...
    parser.add_argument('--diag', action="store_true", help='use diagonal')
...
def main():
    ...
    flows_masks = []
    total = 0
    img_starts = args.img_start
    img_ends = args.img_end
    ratios = args.ratio if args.ratio is not None else [None] * len(img_starts)
    n_steps = len(img_starts)
    if not args.original and not args.diag:
        img_starts = np.repeat(img_starts, n_steps)
        img_ends = np.repeat(img_ends, n_steps)
        ratios = np.tile(ratios, n_steps)
    if args.ratio is not None:
        if len(img_starts) == 1:
            assert len(img_ends) == 1
            img_starts = [img_starts[0]] * len(ratios)
            img_ends = [img_ends[0]] * len(ratios)
    ...
    if len(flows_masks) == 0:
        return

    div_vs_ab = np.full((n_steps, n_steps), fill_value=0, dtype=np.float64)
    div_vs_ba = np.full((n_steps, n_steps), fill_value=0, dtype=np.float64)
    ...
    for idx, img_start, flow, mask, img0, img1 in flows_masks:
        ...
        i = idx // n_steps
        j = idx % n_steps
        div_vs_ab[i, j] = (ab * mask).mean()
        div_vs_ba[i, j] = (ba * mask).mean()
